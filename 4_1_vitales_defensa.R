...
procesar_tabla <- function(x, dx_tabla) {
  ...
  tabla <- tabla %>% 
    filter(!is.na(capitulo_ajustado))
...
tablas_vitales_2 <- lapply(lista_archivos_txt, procesar_tabla, dx_tabla)
...
# Volver todas las listas como una sola tabla

vigencias <- data.frame(vigencia = c(2012:2019))

estad_vitales_2 <- bind_rows(tablas_vitales_2)

estad_vitales_2 <- estad_vitales_2 %>% 
  filter(VIGENCIA %in% vigencias$vigencia)

estad_vitales_unico_2 <- estad_vitales_2 %>% 
  group_by(VIGENCIA, COD_CHIP, capitulo_ajustado) %>% 
  summarise(VALOR = sum(atenciones), .groups = "drop")

estad_vitales_unico_2 <- estad_vitales_unico_2 %>% 
  filter(VIGENCIA %in% vigencias$vigencia)

# Verificamos cuántos Dx sin capítulo - Esperamos 0

estad_caps_na_2 <- estad_vitales_2 %>% 
  filter(is.na(capitulo_ajustado)) 

estad_vitales_unico_2$ESE <- ifelse(estad_vitales_unico_2$COD_CHIP %in% lista_ese$cod_habilitacion, "SI", "NO")

resumen <- estad_vitales_unico_2 %>%
  group_by(VIGENCIA, ESE) %>%
  summarise(suma_valor = sum(VALOR, na.rm = TRUE))

setwd("C:/Users/andre/OneDrive - Universidad de los andes/tesis/data")
write.table(resumen, file = "resumen.txt", sep = "\t", row.names = FALSE, col.names = TRUE, quote = FALSE)