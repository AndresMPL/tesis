...
# Intentamos generar el valor mayor pero los resultados NA perturban más el resultado

efec_tabla <- efec %>% 
  group_by(cod_chip, Indicador, UnidadMedida, vigencia) %>% 
  filter(Fecha == max(Fecha)) %>% 
  summarise(indicador_r = first(indicador_r), .groups = "drop")

# Resumen por año de los que presentaron o no el reporte de cada indicador *

efec_resumen <- efec_tabla %>% 
  group_by(vigencia) %>% 
  summarise(total = n(), 
...
efec_valores <- efec_valores %>% 
  mutate(indicador_i = as.numeric(gsub(",",".", gsub("\\.","", indicador_r))))
...
efect_valores_tabla <- efec_valores %>%  
  group_by(vigencia, Indicador, UnidadMedida) %>% 
  summarise(promedio = mean(indicador_i), .groups="drop")
...
indicadores_inicial_efec <- table(efec_i$Indicador) %>%  
  as.data.frame() # Total indicadores inicial

indicadores_ese_efec <- table(efec_tabla$Indicador) %>%  
  as.data.frame() # Indicadores que aplican a las ESE

indicadores_final_efec <- table(efect_valores_tabla$Indicador) %>%  
  as.data.frame() # Indicadores ESE - reportados
...
exper_valores_tabla <- exper_valores %>%  
  group_by(vigencia, Indicador, UnidadMedida) %>% 
  summarise(promedio = mean(indicador_i), .groups="drop")
...
indicadores_inicial_exper <- table(exper_i$Indicador) %>%  
  as.data.frame() # Total indicadores inicial

indicadores_ese_exper <- table(exper_tabla$Indicador) %>%  
  as.data.frame() # Indicadores que aplican a las ESE

indicadores_final_exper <- table(exper_valores_tabla$Indicador) %>%  
  as.data.frame() # Indicadores ESE - reportados
...
segur_valores_tabla <- segur_valores %>%  
  group_by(vigencia, Indicador, UnidadMedida) %>% 
  summarise(promedio = mean(indicador_i), .groups="drop")
...
indicadores_inicial_segur <- table(segur_i$Indicador) %>%  
  as.data.frame() # Total indicadores inicial

indicadores_ese_segur <- table(segur_tabla$Indicador) %>%  
  as.data.frame() # Indicadores que aplican a las ESE

indicadores_final_segur <- table(segur_valores_tabla$Indicador) %>%  
  as.data.frame() # Indicadores ESE - reportados
